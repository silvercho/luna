...
import study.EndGame.dto.auth.MyAccessDeniedHandler;
import study.EndGame.dto.auth.MyAuthenticationEntryPoint;
import study.EndGame.dto.auth.oauth.PrincipalOauth2UserService;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .antMatchers("/payment/buy/**").authenticated()
                .antMatchers("/security-login/ask/save").authenticated()
                // 인가
