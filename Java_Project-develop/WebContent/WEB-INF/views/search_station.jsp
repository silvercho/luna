...
<script>
	...
	var statnmList = new Array();
	var addrList = new Array();

	var list = JSON.parse('${mapList}');

	for ( var k in list) {
		var $obj = list[k];
		var statnm = $obj.statNm;
		var addr = $obj.addr
		statnmList.push(statnm);
		addrList.push(addr);
	}
	...
	addrList.forEach(function(addr, index) {
		...
		geocoder.addressSearch(addr, function(result, status) {
			...
			if (status === kakao.maps.services.Status.OK) {
				...
				content += '    <a href="charger_information?statNm='
						+ statnmList[index] + '"><strong>' + statnmList[index]
						+ '</strong></a>';
				content += '    <div class="desc">';
				content += '        <span class="address">' + addrList[index]
						+ '</span>';
				content += '    </div>';
				content += '</div>';

				// 인포윈도우 창을 끄는 컨트롤을 표시합니다.
				var iwRemovable = true;

				// 인포윈도우로 장소에 대한 설명을 표시합니다
				var infowindow = new kakao.maps.InfoWindow({
					//  content: statnmList[index], 
					content : content,
					disableAutoPan : true,
