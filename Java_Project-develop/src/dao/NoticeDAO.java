...
public class NoticeDAO {
	...
	//하나의 새로운 게시글이 넘어와서 BD에 저장되는 메서드
		public void insertBoard(BoardVO CVO) {
			...
			try {
			String sql="insert into " + tbl + "(num, writer, subject, content) values(notice_seq.nextval,?,?,?)";
		...
		public void insertreply(BoardVO CVO) {
			...
			//notice에 있는 num과 reply에 있는 num을 group by로 병합하기?
			//다른 컨트롤러에 만들어야 하나
	...
	public ArrayList<BoardVO> AllBoard(Criteria cri){
		...
		try {
			String sql="select * from(select A.*,Rownum Rnum from(select * from notice order by regdate desc)A)" + "where Rnum>? and Rnum<=?";
